		...
		public RouteResult Handle(Api api, RouteRequest request) {
			...
			if (type != null) {
				...
				if (request.Segments.Length > 1) {
					var post = Models.PostModel.GetBySlug(request.Segments[1], type.Slug);
					...
					if (post != null) {
						...
						if (request.Segments.Length > 2 && request.Segments[2] == "comment") {
							route = !String.IsNullOrWhiteSpace(type.CommentRoute) ? type.CommentRoute : "comment/add";

							return new RouteResult(true, true, route, request.Params);
						} else {
							route = !String.IsNullOrWhiteSpace(post.Route) ? post.Route : (!String.IsNullOrWhiteSpace(type.Route) ? type.Route : "post");
							...
							return new RouteResult(true, true, route, request.Params.Concat(new Tuple<string, string>[] { 
								new Tuple<string, string>("id", post.Id.ToString())
							}).ToArray());
						}
					}
				}
				...
				if (type.EnableArchive) {
					...
					request.HttpContext.SetCurrent(new Models.Content() {
						...
						Type = Models.ContentType.Archive
					});

					return new RouteResult(true, true, route, request.Params.Concat(new Tuple<string, string>[] { 
						new Tuple<string, string>("id", type.Id.ToString()),
						new Tuple<string, string>("year", year.ToString()),
						new Tuple<string, string>("month", month.ToString()),
						new Tuple<string, string>("page", page.ToString())
					}).ToArray());
				}
			}
			return new RouteResult(false);
